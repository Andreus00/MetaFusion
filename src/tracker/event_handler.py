...
def handle_event(event, provider, contract, IPFSClient):
    '''
    ...
    kwargs = {k:v for k, v in event_args.items()}
    kwargs['event'] = event_name
    event_object = event_class(**kwargs)
    
    event_object.handle(contract, provider, IPFSClient)
